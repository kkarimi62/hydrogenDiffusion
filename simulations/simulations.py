...
if __name__ == '__main__':
    ...
    def SetVariables():
        Variable = {
                ...
                72:' -var buff 0.0 -var buffy 5.0 -var Tinit 700.0 -var T 700.0 -var seed %s -var nevery 100 -var ParseData 1 -var DataFile data_minimized.txt -var DumpFile dumpThermalized.xyz -var WriteData Equilibrated.dat'%np.random.randint(1001,9999),
                ...
                'p2':' %s 3.54 12.5 8.0 3.0  data_minimized.txt 4 2 1.0 0.0'%(os.getcwd()+'/lmpScripts'),
                'p21':' %s 3.54 52.0 18.0 26.0 data_init.txt 2 2 1.0 0.0'%(os.getcwd()+'/lmpScripts'),
                'p23':' %s 3.54 30.0 14.0 20.0 data_minimized.txt 6 5.0 2 1.0 0.0'%(os.getcwd()+'/lmpScripts'),
...
#                82:[5,'p6',51,72], #--- minimize,add H, minimize, thermalize
                ...
                101:['p23',51, 52, 'p6',51,72], #--- loop, minimize (zero p!), pressurize, add H, minimize (zero p!), kart input, kart.sh to bash shell ,invoke kart
              }[ 101 ]
...
#        print('EXEC=',EXEC)
    ...
    durtn = ['23:59:59','00:14:59','167:59:59'][ 2 ]
    mem = '12gb'
...
#			os.system( 'cp %s/%s %s/lmpScript%s.txt' %( SCRPT_DIR, script, writPath, indx) ) #--- lammps script: periodic x, pxx, vy, load
            os.system( 'cp -s %s/%s %s' %( SCRPT_DIR, script, writPath) ) #--- lammps script: periodic x, pxx, vy, load
        if sourceFiles: 
